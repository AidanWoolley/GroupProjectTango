...
RUN apt update && \
    DEBIAN_FRONTEND=noninteractive apt install -y r-base python3 libxml2-dev libcurl4-openssl-dev libssl-dev
RUN Rscript -e "install.packages(c('lintr', 'rjson', 'gtools', 'yaml'))"


FROM ubuntu:18.04
RUN apt-get update && DEBIAN_FRONTEND=noninteractive apt install -y --no-install-recommends --no-install-suggests r-base python3 python3-pip python3-setuptools libxml2-dev libcurl4-openssl-dev libssl-dev && python3 -m pip install pyyaml && rm -rf /tmp/*
